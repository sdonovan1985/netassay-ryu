...
if ACTIVE_MAPPING == True:
    ...
    from lib.py_timer import py_timer as Timer
...
class DNSMetadataEntry(MetadataEntry):
    ...
    def handle_expiration_callback(self, addr, entry):
        ...
        #TODO
        self.remove_rule_cb(dict(srcip=addr))
        self.remove_rule_cb(dict(dstip=addr))
    ...
    def handle_new_entry_callback(self, addr, entry):
        ...
        for name in entry.names:
            if name == self.rule:
                self.add_rule_cb(dict(srcip=addr))
                self.add_rule_cb(dict(dstip=addr))
        ...
        try:
            ...
            for addr in results:
                ...
                if addr not in self._active_results:
                    self.add_rule_cb(dict(srcip=str(addr)))
                    self.add_rule_cb(dict(dstip=str(addr)))

            # Remove old addresses
            for addr in self._active_results:
                if addr not in results:
                    self.remove_rule_cb(dict(srcip=str(addr)))
                    self.remove_rule_cb(dict(dstip=str(addr)))
